...
function ContactoDetalle({ contacto, onToggleFavorito, onAnteriorContacto, onSiguientContacto }) {
    const mensajeEmpty = (
        <div className="text-text-secondary w-full flex items-center justify-center flex-col h-full">
    ...
    return (
        <div className="mt-2 md:mt-0 border-l-2 border-border pl-4 pr-4 md:pl-2 md:pr-0 flex flex-col h-full">
            <div className="flex md:flex-col border-2 border-border w-full flex-1 rounded-md overflow-hidden">
                {contacto ? (
                    <>
                        <div className="relative flex flex-col w-1/3 md:w-full items-center space-y-2 bg-bg-secondary py-5 flex-shrink-0">
                        ...
                        <div className="flex-1 flex flex-col justify-between py-4 min-h-0">
                            {/* Detalles */}
                            <div className="flex-1 space-y-3 text-sm flex flex-col items-start px-4 md:px-0 md:items-center  text-text-primary">
                                <p>
                                    <strong className="text-secondary-neutral">Teléfono:</strong> {contacto.telefono}
                                </p>
                                {contacto.correo && (
                                    <p>
                                        <strong className="text-secondary-neutral">Correo:</strong> {contacto.correo}
                                    </p>
                                )}
                                {contacto.direccion && (
                                    <p>
                                        <strong className="text-secondary-neutral">Dirección:</strong> {contacto.direccion}
                                    </p>
                                )}
                            </div>
                            ...
                            <div className="flex justify-center gap-2 flex-shrink-0">
                                <button onClick={onAnteriorContacto} className="hover:opacity-70 transition-opacity">
                                ...
                                <button onClick={onSiguientContacto} className="hover:opacity-70 transition-opacity">
                                    <svg className="size-9" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
                                        <g id="SVGRepo_tracerCarrier" strokeLinecap="round" strokeLinejoin="round"></g>
                                        <g id="SVGRepo_iconCarrier">
